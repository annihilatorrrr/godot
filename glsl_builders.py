        ...
        while line:
            ...
            while line.find("#include ") != -1:
                ...
                if includeline.startswith("thirdparty/"):
                    ...
                    included_file = os.path.relpath(f"{os.path.dirname(filename)}/{includeline}")
            ...
            elif header_data.reading == "fragment":
                header_data.fragment_lines += [line]
            elif header_data.reading == "vertex":
...
def build_rd_header(filename):
    ...
    with open(out_file, "w") as fd:
        ...
        fd.write(f"#ifndef {out_file_ifdef}" + "_RD\n")
        fd.write(f"#define {out_file_ifdef}" + "_RD\n")
        ...
        fd.write(f"class {out_file_class}" + " : public ShaderRD {\n\n")
        fd.write("public:\n\n")
...
def build_raw_header(filename):
    ...
    with open(out_file, "w") as fd:
        ...
        out_file_base = out_file_base[out_file_base.rfind("\\") + 1 :]
        out_file_ifdef = out_file_base.replace(".", "_").upper()
        fd.write(f"#ifndef {out_file_ifdef}" + "_RAW_H\n")
        fd.write(f"#define {out_file_ifdef}" + "_RAW_H\n")
        fd.write("\n")
        fd.write(f"static const char {out_file_base}" + "[] = {\n")
