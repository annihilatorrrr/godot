...
def make_doc_header(target, source, env):
    ...
    with open(dst, "w", encoding="utf-8") as g:
        ...
        g.write(f"static const int _doc_data_compressed_size = {len(buf)}" + ";\n")
        g.write(
            f"static const int _doc_data_uncompressed_size = {decomp_size}"
            + ";\n"
        )
...
def make_translations_header(target, source, env, category):
    ...
    with open(dst, "w", encoding="utf-8") as g:
        ...
        for i in range(len(sorted_paths)):
            if msgfmt_available:
                mo_path = os.path.join(tempfile.gettempdir(), f"{uuid.uuid4().hex}.mo")
                cmd = f"msgfmt {sorted_paths[i]} --no-hash -o {mo_path}"
                try:
            ...
            g.write(
                f"static const unsigned char _{category}_translation_{name}_compressed[] = {{\n"
            )
        ...
        g.write("};\n\n")
        g.write(
            f"static {category.capitalize()}TranslationList _{category}_translations[] = {{\n"
        )
